...
boru-driven command-line client **and** interactive REPL for the ApiTools
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/api-tools-cli)
make build
...
API_TOOLS_BASE=https://api.example.com ./api-tools-cli --help
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/api-tools-cli
   ```
...
Run with no arguments to open a REPL (prompt `api-tools>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `cryptography`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/api-tools-cli`. |
...
(`./api-tools-cli <expr>`) and interactively (the REPL), and expressions compose the
