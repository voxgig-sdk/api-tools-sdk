// Command ApiTools-cli is a CLI + REPL for the ApiTools SDK,
...
const prompt = "api-tools"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "cryptography encoding generator get_documentation tool utility"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      API_TOOLS_APIKEY (api key), API_TOOLS_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: API_TOOLS_APIKEY carries the API key and
	// API_TOOLS_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("API_TOOLS_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("API_TOOLS_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewApiToolsSDK(opts)
