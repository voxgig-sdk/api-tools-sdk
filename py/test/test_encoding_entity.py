...
class TestEncodingEntity:

    def test_should_create_instance(self):
        testsdk = ApiToolsSDK.test(None, None)
        ent = testsdk.Encoding(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _encoding_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "encoding." + _op, "live" if _live else "unit")
            if _skip:
        ...
        # CREATE
        encoding_ref01_ent = client.Encoding(None)
        encoding_ref01_data = helpers.to_map(vs.getprop(
            vs.getpath(setup["data"], "new.encoding"), "encoding_ref01"))

        encoding_ref01_data_result, err = encoding_ref01_ent.create(encoding_ref01_data, None)
        assert err is None
...
def _encoding_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/encoding/EncodingTestData.json")
