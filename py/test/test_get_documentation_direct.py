...
class TestGetDocumentationDirect:

    def test_should_direct_list_get_documentation(self):
        setup = _get_documentation_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-get_documentation", "live" if setup["live"] else "unit")
        if _skip:
...
def _get_documentation_direct_setup(mockres):
