...
class TestToolDirect:
    ...
    def test_should_direct_list_tool(self):
        setup = _tool_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-tool", "live" if setup["live"] else "unit")
        if _skip:
