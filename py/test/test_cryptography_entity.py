...
class TestCryptographyEntity:

    def test_should_create_instance(self):
        testsdk = ApiToolsSDK.test(None, None)
        ent = testsdk.Cryptography(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _cryptography_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "cryptography." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set API_TOOLS_TEST_CRYPTOGRAPHY_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        cryptography_ref01_ent = client.Cryptography(None)
        cryptography_ref01_data = helpers.to_map(vs.getprop(
            vs.getpath(setup["data"], "new.cryptography"), "cryptography_ref01"))

        cryptography_ref01_data = helpers.to_map(runner.entity_data(cryptography_ref01_ent.create(cryptography_ref01_data, None)))
        assert cryptography_ref01_data is not None



def _cryptography_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/cryptography/CryptographyTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "API_TOOLS_TEST_CRYPTOGRAPHY_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("API_TOOLS_TEST_CRYPTOGRAPHY_ENTID"))
