...
class GetDocumentationEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return GetDocumentationEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[GetDocumentation]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.GetDocumentation().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
