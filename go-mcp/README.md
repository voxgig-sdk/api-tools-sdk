...
[MCP](https://modelcontextprotocol.io) server exposing the ApiTools SDK as
two agent tools — `api-tools_list` and `api-tools_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/api-tools-mcp)
make build

# 2. Provide credentials via the environment
export API_TOOLS_APIKEY=sk_live_xxx
...
```jsonc
// api-tools_list: first page of records
{ "entity": "generator" }
{ "entity": "generator", "query": { } }

// api-tools_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/api-tools-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user api-tools \
     -- "$PWD"/dist/*/api-tools-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `api-tools_list` and `api-tools_load` tools now appear
   in new sessions. Ask the agent to *"list generator using api-tools"*
   and it calls `api-tools_list` with `{"entity":"generator"}`.
...
```sh
export API_TOOLS_APIKEY=sk_live_xxx            # API key
export API_TOOLS_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `api-tools_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `api-tools_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `API_TOOLS_APIKEY` | API key sent with every request. |
| `API_TOOLS_BASE` | Optional override of the API base URL. |
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"api-tools_load","arguments":{"entity":"generator","query":{"id":1}}}}'
```
