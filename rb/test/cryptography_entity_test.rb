...
class CryptographyEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set API_TOOLS_TEST_CRYPTOGRAPHY_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    cryptography_ref01_data_result = cryptography_ref01_ent.create(cryptography_ref01_data, nil)
    cryptography_ref01_data = Helpers.to_map(cryptography_ref01_data_result.respond_to?(:data_get) ? cryptography_ref01_data_result.data_get : cryptography_ref01_data_result)
...
def cryptography_basic_setup(extra)
  ...
  entid_env_raw = ENV["API_TOOLS_TEST_CRYPTOGRAPHY_ENTID"]
