...
class EncodingEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = encoding_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "encoding." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    # CREATE
    encoding_ref01_ent = client.Encoding(nil)
...
def encoding_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "encoding", "EncodingTestData.json")
