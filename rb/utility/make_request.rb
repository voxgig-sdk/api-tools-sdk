...
module ApiToolsUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = ApiToolsResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ApiToolsResponse.new(fetched)
    else
