...
class GetDocumentationEntity
  ...
  def make
    opts = @_entopts.dup
    GetDocumentationEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = ApiToolsHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [GetDocumentation, Hash] the current GetDocumentation data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = ApiToolsHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of GetDocumentation fields)
  def match_get
  ...
  #
  # @param reqmatch [GetDocumentationListMatch, Hash, nil] match filter (any subset of
  #   GetDocumentation fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<GetDocumentation>, Array] the matching GetDocumentation items; raises ApiToolsError on failure
