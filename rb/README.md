...
The Ruby SDK for the ApiTools API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
# Create
created, _ = client.Cryptography(nil).create({ "name" => "Example" }, nil)
...
result, err = client.ApiTools(nil).load(
  { "id" => "test01" }, nil
)
