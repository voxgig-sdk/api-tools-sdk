...
local EncodingEntity = {}
EncodingEntity.__index = EncodingEntity


function EncodingEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, EncodingEntity)
  self._name = "encoding"
  self._client = client
...
function EncodingEntity:get_name()
  return self._name
end


function EncodingEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return EncodingEntity.new(self._client, opts)
end
...
function EncodingEntity:mark_deleted()
  self._deleted = true
end


function EncodingEntity:deleted()
  return true == self._deleted
end


function EncodingEntity:data_set(args)
  if args ~= nil then
...
function EncodingEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function EncodingEntity:match_set(args)
  if args ~= nil then
...
function EncodingEntity:match_get()
...
function EncodingEntity:stream(action, args, callopts)
...
function EncodingEntity:_run_op(ctx, post_done)
  local utility = self._utility
