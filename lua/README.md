...
The Lua SDK for the ApiTools API. Provides an entity-oriented interface using Lua conventions.
...
```lua
-- Create
local created, _ = client:Cryptography(nil):create({ name = "Example" }, nil)
...
local result, err = client:ApiTools(nil):load(
  { id = "test01" }, nil
)
...
```
lua/
├── api-tools_sdk.lua    -- Main SDK module
...
The main module (`api-tools_sdk`) exports the SDK constructor
