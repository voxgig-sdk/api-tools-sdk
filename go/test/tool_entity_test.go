...
func TestToolEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Tool(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Tool(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set API_TOOLS_TEST_TOOL_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		toolRef01Match := map[string]any{}

		toolRef01ListResult, err := toolRef01Ent.List(toolRef01Match, nil)
		if err != nil {
			t.Fatalf("list failed: %v", err)
		}
		_, toolRef01ListOk := toolRef01ListResult.([]any)
		if !toolRef01ListOk {
			t.Fatalf("expected list result to be an array, got %T", toolRef01ListResult)
		}
...
func toolBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("API_TOOLS_TEST_TOOL_ENTID")
	...
	idmapResolved := core.ToMapAny(env["API_TOOLS_TEST_TOOL_ENTID"])
