...
func TestUtilityEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set APITOOLS_TEST_UTILITY_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func utilityBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("APITOOLS_TEST_UTILITY_ENTID")
	...
	idmapResolved := core.ToMapAny(env["APITOOLS_TEST_UTILITY_ENTID"])
