...
import (
	...
	omni "github.com/voxgig-sdk/api-tools-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ApiToolsError); is {
				out := map[string]any{
					"name":    "ApiToolsError",
...
func omniCtx(arg any, client *sdk.ApiToolsSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ApiToolsSDK); is {
			client = live
