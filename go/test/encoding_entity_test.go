...
func TestEncodingEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set APITOOLS_TEST_ENCODING_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		encodingRef01Data = core.ToMapAny(encodingRef01DataResult)
...
func encodingBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("APITOOLS_TEST_ENCODING_ENTID")
