...
func TestGeneratorEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set APITOOLS_TEST_GENERATOR_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		generatorRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.generator", setup.data)))
		var generatorRef01Data map[string]any
		if len(generatorRef01DataRaw) > 0 {
			generatorRef01Data = core.ToMapAny(generatorRef01DataRaw[0][1])
		}
		// Discard guards against Go's unused-var check when the flow's steps
		// happen not to consume the bootstrap data (e.g. list-only flows).
		_ = generatorRef01Data
...
func generatorBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("APITOOLS_TEST_GENERATOR_ENTID")
	...
	idmapResolved := core.ToMapAny(env["APITOOLS_TEST_GENERATOR_ENTID"])
