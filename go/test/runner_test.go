...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("API_TOOLS_TEST_LIVE") == "TRUE" ||
		os.Getenv("API_TOOLS_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("API_TOOLS_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.ApiToolsSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.ApiToolsError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.ApiToolsSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.ApiToolsError{Msg: msg, Code: code}
}
