...
//     (\`new ApiToolsSDK(...)\` / \`ApiToolsSDK.test(...)\` become
//     \`ApiToolsSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"cryptography":{"test01":{"id":"test01"}},"encoding":{"test01":{"id":"test01"}},"generator":{"test01":{"id":"test01"}},"get_documentation":{"test01":{"id":"test01"}},"tool":{"test01":{"id":"test01"}},"utility":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new ApiToolsSDK(...)\` and any
// \`ApiToolsSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+ApiToolsSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/ApiToolsSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
