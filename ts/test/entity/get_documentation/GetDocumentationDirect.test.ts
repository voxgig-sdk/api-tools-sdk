...
describe('GetDocumentationDirect', async () => {
  ...
  test('direct-list-get_documentation', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-get_documentation', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['API_TOOLS_TEST_GET_DOCUMENTATION_ENTID']
