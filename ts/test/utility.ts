...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.APITOOLS_TEST_LIVE ||
    'TRUE' === process.env.APITOOLS_TEST_OVERRIDE
  ) {
  ...
  m.APITOOLS_TEST_EXPLAIN = process.env.APITOOLS_TEST_EXPLAIN || m.APITOOLS_TEST_EXPLAIN
