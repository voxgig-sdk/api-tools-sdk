...
(0, node_test_1.describe)('ToolDirect', async () => {
    ...
    (0, node_test_1.test)('direct-list-tool', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-tool', setup.live))
            return;
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['API_TOOLS_TEST_TOOL_ENTID'];
