...
class GetDocumentationEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new GetDocumentationEntity($this->_client, $opts);
    }

    /**
     * @param GetDocumentation|array $args GetDocumentation data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = ApiToolsHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return GetDocumentation|array The current GetDocumentation data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of GetDocumentation fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = ApiToolsHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of GetDocumentation fields).
     */
    ...
    /**
     * List GetDocumentation items matching the given filter.
     *
     * @param GetDocumentationListMatch|array|null $reqmatch Match filter (any subset
     *   of GetDocumentation fields) as an assoc-array; GetDocumentationListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return GetDocumentation[]|array A list of GetDocumentation items as assoc-arrays at
     *   the SDK boundary; throws ApiToolsError on failure (item-5 convention).
     */
