...
class FtCtrl extends ApiToolsControl
{
...
class FtClient
{
    ...
    private ApiToolsUtility $utility;

    public function __construct(ApiToolsUtility $utility, array $options)
    {
    ...
    public function get_utility(): ApiToolsUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ApiToolsContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ApiToolsContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ApiToolsContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ApiToolsConfig::make_feature($name);
    ...
    private static function buildUrl(ApiToolsSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ApiToolsError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ApiToolsResponse($fetched);
        }
    ...
    private function fail(ApiToolsContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ApiToolsContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
