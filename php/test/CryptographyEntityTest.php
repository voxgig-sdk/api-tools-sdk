...
class CryptographyEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = ApiToolsSDK::test(null, null);
        $ent = $testsdk->Cryptography(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = cryptography_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "cryptography." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        // CREATE
        $cryptography_ref01_ent = $client->Cryptography(null);
...
function cryptography_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/cryptography/CryptographyTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["cryptography01", "cryptography02", "cryptography03"] as $k) {
