...
class ApiToolsNetsimFeature extends ApiToolsBaseFeature
{
    ...
    private function _simulate(ApiToolsContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(ApiToolsContext $ctx, array $applied): void
    {
        $client = $this->client;
